...
jQuery(document).ready(function($) {
	...
	M.getDB('filepath', GUI.currentpath, 'file');

	// Load library
	$("#lib-loader").show();
	$.post('db/?cmd=loadlib', {}, function(json) {
		$("#lib-loader").hide();
		$("#lib-content").show();
		loadLibrary(json);
	}, 'json');
	...
	M.readTcmConf().done(function(json) {
		M.log('[main] done reading conf', json);
 ...
 * TODO the more complex ones should become separate functions
 */
// AG
function attachHandlers() {
	// BUTTON CLICK HANDLERS
	$('.btn-cmd').click(function() {
		var cmd, vol, volevent;
		...
		if ($(this).hasClass('btn-toggle')) {
			...
			if ($(this).attr('id') == 'prev' && parseInt(GUI.json.time) > 0 && parseInt(getMpdStatus()['elapsed']) > 0) {
				...
				M.sendMpdCmd('seek ' + GUI.json.song + ' ' + 0);
				if (GUI.state != 'pause') {
					cmd = 'pause';
				} else {
					cmd = '';
	...
	$('#toolbar-btn').click(function() {
		...
		if ($('#open-panel-sx').hasClass('active')) {
			...
			if ($('#db-currentpath span').text() === '' || $('#db-currentpath span').text().substr(0, 3) == 'NAS') {
				if (TCMCONF.json.search_autofocus_enabled == 'Yes') {
	...
	$('#countdown-display').click(function() {
		// Toggle the setting
		TCMCONF.json.time_knob_countup = (TCMCONF.json.time_knob_countup == "1") ? "0" : "1";
	...
	$('.playlist').on('click', '.pl-action', function() {
		...
		if ($(window).height() - ($(this).offset().top - $(window).scrollTop()) <= relOfs) {
			$('#context-menus .dropdown-menu').css({
				"top": posTop
			}); // 3 menu items
		} else {
			$('#context-menus .dropdown-menu').css({
	...
	$('.database').on('click', '.db-browse', function() {
		...
		if (!$(this).hasClass('sx')) {
			...
			if ($(this).hasClass('db-folder')) {
				...
				entryID = $(this).parent().attr('id').replace('db-', '');
	...
	$('.btn-delete-plitem').click(function() {
		...
		var cmd = 'trackremove&songid=' + begpos;
		if (begpos !== endpos) {
			cmd += ':' + (endpos + 1);
		}
		M.notify('remove');
	...
	$('.btn-move-plitem').click(function() {
		...
		var cmd = 'trackmove&newpos=' + newpos + '&songid=' + begpos;
		if (begpos !== endpos) {
			cmd += ':' + (endpos + 1);
		}
		M.notify('move');
...
function createKnobs() {
	...
	$('.playbackknob').knob({
		...
		change: function(value) {
			if (GUI.state !== 'stop') {
				...
				if (TCMCONF.json.time_knob_countup == "1" || parseInt(GUI.json.time) === 0) {
					$('#countdown-display').html(timeConvert(seekto)); // count up
				} else {
					$('#countdown-display').html(timeConvert(parseInt(GUI.json.time) - seekto)); // count down
				}
			} else {
				$('#time').val(0);
			}
		...
		release: function(value) {
			if (GUI.state !== 'stop') {
				...
				window.clearInterval(GUI.currentKnob);
				var seekto = Math.floor((value * parseInt(GUI.json.time)) / 1000);
				M.sendMpdCmd('seek ' + GUI.json.song + ' ' + seekto);
	...
	$('.volumeknob').knob({
		...
		release: function(value) {
			// TC (Tim Curtis) 2015-01-27: not needed, results in duplicate setVolume cmds
		},
		cancel: function() {},
		draw: function() {
			// Skin "tron"
