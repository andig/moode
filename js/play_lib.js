...
function makeCoverURL(filepath) {
	// (1) Remove /songfilename.ext and build partial path
	cutpos = filepath.lastIndexOf("/");
	if (cutpos != -1) {
		var coverurl = MPDCS.coverroot + filepath.slice(0,cutpos);
	} else {
		var coverurl = MPDCS.defaultcover;
	}
	
	// (2) Add /imagefile.ext to path, see which one if any exists
	if (coverurl != MPDCS.defaultcover) {
		coverurl = coverurl.replace("#", "%23") // Replace a few troublesome characters with escape codes
		
		if (TCMCONF.json['albumart_lookup_method'] == "Default" || TCMCONF.json['albumart_lookup_method'] == "All") {
			// TC (Tim Curtis) 2014-10-31: check both folder and Folder, cover and Cover...
			var found = true;

			if (coverImgExists(coverurl + MPDCS.folderjpg)) { // Folder (uppercase F)
				coverurl = coverurl + MPDCS.folderjpg;
			} else if (coverImgExists(coverurl + MPDCS.folderjpgl)) { // folder (lowercase f)
				coverurl = coverurl + MPDCS.folderjpgl;
			} else if (coverImgExists(coverurl + MPDCS.coverjpg)) { // Cover
				coverurl = coverurl + MPDCS.coverjpg;
			} else if (coverImgExists(coverurl + MPDCS.coverjpgl)) { // cover
				coverurl = coverurl + MPDCS.coverjpgl;
			} else if (coverImgExists(coverurl + MPDCS.folderpng)) {
				coverurl = coverurl + MPDCS.folderpng;
			} else if (coverImgExists(coverurl + MPDCS.folderpngl)) {
				coverurl = coverurl + MPDCS.folderpngl;
			} else if (coverImgExists(coverurl + MPDCS.coverpng)) {
				coverurl = coverurl + MPDCS.coverpng;
			} else if (coverImgExists(coverurl + MPDCS.coverpngl)) {
				coverurl = coverurl + MPDCS.coverpngl;
			} else {
				coverurl = MPDCS.defaultcover;
				found = false;
			}
		} else {found = false;}
		
		if (TCMCONF.json['albumart_lookup_method'] == "EAC" || TCMCONF.json['albumart_lookup_method'] == "All") {
			if (found == false) {
				console.log('coverurl= ', coverurl);
				if (coverImgExists(coverurl + "/" + MPDCS.json['AlbumArtist'] + " - " + MPDCS.json['Album'] + ".jpg")) {
					coverurl = coverurl + "/" + MPDCS.json['AlbumArtist'] + " - " + MPDCS.json['Album'] + ".jpg";
					found = true;
				} else {
					coverurl = MPDCS.defaultcover;
					found = false;
		...
		if (TCMCONF.json['albumart_lookup_method'] == "Embedded" || TCMCONF.json['albumart_lookup_method'] == "All") {
			if (found == false) {
				// place holder
				coverurl = MPDCS.defaultcover;
				/*
				if (extractImage(songFile) != '') {
					coverurl = "path to image file";
				} else {
					coverurl = MPDCS.defaultcover;
				}
				*/
...
function makeUPNPCoverURL() {
	...
	/* This work around was not effective since the bogus url actually returned a <head></head>, very odd
	if (upnpCoverUrl.substr(0,4) != "http") {
		return MPDCS.defaultcover;
	} else {
		if (coverImgExists(upnpCoverUrl)) { //work around when bogus url is returned by upexplorer
			return upnpCoverUrl;
		} else {
			return MPDCS.defaultcover;
		}
