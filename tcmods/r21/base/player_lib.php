...
function loadAllLib($sock) {
	...
	$debug_fhand = fopen("/var/www/liblog.txt", 'w'); // create, clear contents of file
	fwrite($debug_fhand, "debug flags= ".$debug_flags[0].",".$debug_flags[1].",".$debug_flags[2].",".$debug_flags[3].",".$debug_flags[4]."\n"); // write out the debug flags
	fclose($debug_fhand);

	$flat = _loadDirForLib($sock, array(), "", $debug_flags);

	// TC (Tim Curtis) 2015-06-26: debug, #0 total count of files
	if ($debug_flags[0] == "y") {
		$debug_fhand = fopen("/var/www/liblog.txt", 'a'); // append
		fwrite($debug_fhand, "_loadAllLib() count= ".count($flat)."\n");
		fclose($debug_fhand);
	}
	
	return json_encode(_organizeJsonLib($flat, $debug_flags));
}

// TC (Tim Curtis) 2015-06-26: add debug logging
// TC (Tim Curtis) 2015-07-31: let MPD generate the flat list via the find command
function _loadDirForLib($sock, $flat, $dir, $debug_flags) { // remove $dir in final version...
	if ($debug_flags[4] == "1") {
	...
	if (!is_null($resp) && substr($resp, 0, 2) != "OK") {
		...
		for ($iLine = 0; $iLine < count($lines); $iLine++) {
			...
			if ($element == "file") {
				...
				if ($debug_flags[1] == "y") {
					$debug_fhand = fopen("/var/www/liblog.txt", 'a'); // append
					fwrite($debug_fhand, "_loadDirForLib() item= ".$item.", file= ".$value."\n");
					fclose($debug_fhand);
				}
			}
			...
			if ($debug_flags[2] == "y") {
				$debug_fhand = fopen("/var/www/liblog.txt", 'a'); // append
				fwrite($debug_fhand, "_loadDirForLib() item= ".$item.", element= ".$element.", value= ".$value."\n");
				fclose($debug_fhand);
			}
	...
	if (!is_null($resp)) {
		...
		for ($iLine = 0; $iLine < count($lines); $iLine++) {
			...
			if ($element == "file") {
				...
				$flat = _loadDirForLib($sock, $flat, $value, $debug_flags);
				$skip = true;
...
// TC (Tim Curtis) 2015-01-01: add time (secs) and time2 (mm:ss) to songDataLight 
// TC (Tim Curtis) 2015-06-26: add debug logging
function _organizeJsonLib($flat, $debug_flags) {
	$item = 0; // debug
	...
	foreach ($flat as $songData) {
		...
		$songDataLight = array("file" => $songData['file'], "display" => ($songData['Track'] ? $songData['Track']." - " : "").$songData['Title'], "time" => $songData['Time'], "time2" => songTime($songData['Time']));
		array_push($lib[$genre][$artist][$album], $songDataLight);

		// TC (Tim Curtis) 2015-06-26: debug, #3 songDataLight file name
		if ($debug_flags[3] == "y") {
			$debug_fhand = fopen("/var/www/liblog.txt", 'a'); // append
			fwrite($debug_fhand, "_organizeJsonLib() songDataLight= ".$item++.", ".$songDataLight['file']."\n");
			fclose($debug_fhand);
		}
	}
...
// original, not working correctly for 3.18.5+
/*
function getMixerName($kernelver, $i2s) {
	if ($kernelver == '3.18.5+' || $kernelver == '3.18.11+' || $kernelver == '3.18.14+') {
		if ($i2s != 'I2S Off') {
			if ($i2s == 'HiFiBerry Amp(Amp+)') {
				...
				$mixername = 'Digital'; // other i2s devices
			}
		} else {
			$mixername = 'PCM'; // USB devices
		}
	} else {
		$mixername = 'PCM'; // default if using other kernels
	}
